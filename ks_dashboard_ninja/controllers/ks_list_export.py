...
class KsListExport(ExportFormat, http.Controller):
    ...
    def base(self, data, token):
        ...
        header, list_data, item_id, ks_export_boolean, context = operator.itemgetter('header','chart_data','ks_item_id',
                                                                                     'ks_export_boolean','context')(params)
        ...
        if ks_export_boolean:
            ...
            ks_chart_domain = item.ks_convert_into_proper_domain(item.ks_domain, item)
            if list_data['type'] == 'ungrouped':
                ...
                ks_query_result = item.ks_get_list_query_result(ks_query,query_start_date,query_end_date, ks_offset=0, ks_export_all=True)
